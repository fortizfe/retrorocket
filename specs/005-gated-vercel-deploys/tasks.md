...
- [X] T007 [US1] In the same `deploy-preview` job, add steps to install the Vercel CLI and run `vercel pull --yes --environment=preview --token=$VERCEL_TOKEN`, `vercel build`, `vercel deploy --prebuilt`, using the `VERCEL_TOKEN`/`VERCEL_ORG_ID`/`VERCEL_PROJECT_ID` secrets from Phase 1 (research.md §4, FR-001, FR-003) (depends on T006). **Done, amended twice after live runs**: (1) `vercel build` alone did not install project dependencies (`vite: not found`); added an explicit `npm ci` step (with npm caching, matching `ci.yml`'s pattern). (2) That still failed the same way — `vercel build` execs the buildCommand directly via `sh -c`, bypassing `npm run`'s PATH prepending, and each Actions `run:` step is a fresh shell — fixed by appending `node_modules/.bin` to `$GITHUB_PATH` in the same step as `npm ci`. See research.md §4 correction notes.
