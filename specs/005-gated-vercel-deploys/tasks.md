...
- [X] T009 [US1] Open a pull request with passing tests and a clean CodeQL scan; confirm `deploy-preview` waits for all three checks, then deploys, and that the preview link is discoverable on the PR (quickstart.md Part 1) (depends on T008). **Done, verified live on PR #8 (2026-07-21)**: all three checks (`Type-check, lint, and test with coverage`, `Playwright E2E (Firebase Emulator Suite)`, `CodeQL`) passed, `Deploy Preview` ran to completion, and the sticky comment posted `Preview deployment: https://retro-rocket-nfmbxw69b-fernando-ortizs-projects-98fdba97.vercel.app`. Took three amendment rounds to get here — see T007's notes and research.md §4 correction notes for the `npm ci`/PATH/working-directory fixes discovered along the way.
- [ ] T010 [US1] On the same PR, push a commit that fails lint (or introduces a High-severity CodeQL finding); confirm `deploy-preview` does not run to completion and no preview deployment is created for that commit (quickstart.md Part 2; FR-001, FR-005) (depends on T009). **Not yet done**: T009 just succeeded live; this negative-path test hasn't been run yet in this session.
- [X] T011 [US1] Confirm the successful preview deployment from T009 is reachable only via its preview URL and has no effect on the production environment (`vercel list retro-rocket --environment=production`) (quickstart.md Part 1 step 5; FR-003, SC-003) (depends on T009). **Done**: `Deploy Production` reported `skipping` on this same PR run (its `if: github.event_name == 'push'` condition correctly excludes `pull_request` events) — direct evidence no production deployment was triggered by this preview.
