...
**Third correction after live run (2026-07-21)**: with `vite` now found, the build failed differently — `Could not resolve entry module "index.html"` with "0 modules transformed" (an effectively empty build). Root cause: the Vercel project's dashboard-configured "Root Directory" is `retro-rocket` (required for Vercel's own git-triggered builds, which always start from the repo root, to find the app at all). `vercel build`/`vercel deploy` apply that Root Directory offset themselves — they're meant to be invoked from the actual repository root, not from inside the already-nested `retro-rocket/` directory, or the offset gets applied twice (looking for `retro-rocket/retro-rocket/index.html`, which doesn't exist). Fix: removed the job-level `defaults.run.working-directory: retro-rocket` from `deploy.yml` entirely; only the `npm ci`/PATH step now explicitly sets `working-directory: retro-rocket` (matching where `package-lock.json` lives), while `vercel pull`/`build`/`deploy` run from the actual repository root and let the CLI apply the Root Directory offset itself.
