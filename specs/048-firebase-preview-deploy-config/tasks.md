...
- [X] T017 [US2] Wire `assign-alias.mjs` into `.github/workflows/ci.yml`'s `deploy-preview` job per `specs/048-firebase-preview-deploy-config/contracts/pr-preview-alias-cli.md` so `OAUTH_REDIRECT_BASE_URL` is set correctly for this PR's deployment (depends on T010–T014, T016). **Done, after two real bugs found on live runs — the second one only surfaced when the user manually clicked "sign in" on the deployed preview**:
  1. First bug (path): the original "compute" step wrote to `retro-rocket/.vercel/.env.preview.local`, but `vercel pull` in this job actually writes it at the **repo root**. Fixed by pointing at the right path.
  2. Second, deeper bug (mechanism): even with the right path, appending a line to the locally-pulled env file before `vercel build` has **zero effect** on the deployed Function's runtime — confirmed by inspecting a real local build's `.vercel/output/functions/api/index.func/.vc-config.json`, whose `environment` field was completely empty. Vercel injects a Function's runtime env vars from its own stored project config at deploy time, not from anything in the build output. Rewrote the mechanism entirely: `assign-alias.mjs`'s `compute` subcommand became `set-redirect`, which actually calls `vercel env rm`/`vercel env add` to set `OAUTH_REDIRECT_BASE_URL` as a real, branch-scoped Vercel-stored Preview variable before `vercel pull`; `vercel pull` gained `--git-branch=${{ github.head_ref }}`; `vercel deploy` gained `-m githubDeployment=1 -m githubCommitRef=${{ github.head_ref }}` to link the CLI deployment to its branch (without which the branch-scoped override never resolves). Verified live outside CI: built + deployed a real throwaway preview with this exact sequence, then `vercel curl .../api/auth/login/google` correctly redirected with `redirect_uri=https://retro-rocket-pr-slot-1.vercel.app/...` and the right Google Client ID. Throwaway deployments and test env vars cleaned up afterward. Tests rewritten (still TDD: red before green), `contracts/pr-preview-alias-cli.md` and `research.md` §1/§3 corrected.
- [ ] T018 [US2] On a test pull request's preview (T008's PR, redeployed after T017 merges, or a new one), sign in with Google, confirm the sign-in completes and lands in an authenticated session, and record the elapsed time from the preview being reported ready to the completed sign-in, confirming it is under 2 minutes (`quickstart.md` Scenario 2, steps 1–2; spec SC-002; depends on T017). **Mechanically verified, full click-through still pending.** On PR #65's actual deployment (`retro-rocket-f7ixdjfng-...`), `GET /api/auth/login/google` correctly redirects to Google's consent screen with the right `client_id` and `redirect_uri=https://retro-rocket-pr-slot-1.vercel.app/api/auth/callback/google` — the exact URI the user registered in T014. What's not yet done: actually clicking through Google's consent screen with a real account and confirming the callback lands in an authenticated session — needs a human (or credentials I don't have) to complete.
- [ ] T019 [US2] On the same preview, sign in with GitHub and confirm the sign-in completes and lands in an authenticated session (`quickstart.md` Scenario 2, step 3; depends on T017). **Mechanically verified, full click-through still pending** — same as T018: `GET /api/auth/login/github` correctly redirects with the right `client_id` (`Ov23liEJSzzCdrm5SzNX`) and slot-1 redirect URI; the actual consent-screen click-through needs a human.
