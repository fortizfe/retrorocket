...
- [ ] T008 [US1] Open a test pull request, wait for `analyze`/`checks`/`e2e`/`deploy-preview`/`sync-preview-domain` to succeed, and confirm its preview URL loads with no environment-configuration error (`quickstart.md` Scenario 1, steps 1–4; depends on T006, T007). **BLOCKED**: PR [#65](https://github.com/fortizfe/retrorocket/pull/65) opened and CI run 5 times; `analyze`/`checks` (CodeQL, type-check/lint/test) pass every time, but the required `e2e` job has failed all 5 runs on a shifting set of pre-existing, unrelated tests (`concurrent-board-session.spec.ts` reconnect, `retrospective-board.spec.ts` typing-indicator-on-disconnect, and the `isAnchoredToTrigger()`-based dropdown-position specs 044/045) — never anything this feature's diff touches (only `ci.yml`'s `deploy-preview` job and a new `scripts/` file; the `e2e` job itself is unmodified). Confirmed `main`'s own last push run was green with the same suite. Since `deploy-preview` needs `[analyze, checks, e2e]` all green, it has not run yet on this PR, so T008/T009 and everything downstream that depends on a live preview (T018–T023) are blocked on this pre-existing CI flakiness, not on this feature's changes. Stopped retrying after 5 attempts per explicit user direction rather than continuing indefinitely.
- [ ] T009 [US1] On that same preview, create/edit a retro board and confirm the write appears in `retrorocket-staging`'s Firestore (Firebase console) and never in the production project (`quickstart.md` Scenario 1, steps 5–6; depends on T008). **Blocked on T008.**
