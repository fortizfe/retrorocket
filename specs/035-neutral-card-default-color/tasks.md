...
- [ ] T007 [US2] No production code change required — `ColorPicker` and `selectedColor`/`setSelectedColor` wiring in `GroupableColumn.tsx` are untouched by T003. Manually verify per `quickstart.md` §3 ("Manual override still works") in the browser: pick a non-default color for a new card and confirm it persists, then edit an existing card's color and confirm that still works too. **Not performed as a live browser session** — same infra constraint as T010 (requires Firebase Auth/Firestore emulators + backend). Automated coverage instead: `GroupableColumn.test.tsx`'s "should update the selected color when the user picks one manually" and "should create card with the manually selected color when the user overrides the default" tests exercise this exact flow through real component code via RTL `user-event`, and both pass.
...
- [X] T009 [P] Confirm no E2E update is needed: `e2e/retrospective-board.spec.ts` has only one `color` reference (line ~1319, an unrelated hardcoded `'pastelBlue'` fixture for a pre-existing legacy card in a different scenario) per research.md Decision 4 — grep the `e2e/` directory to reconfirm no test asserts column-derived default coloring on the add-card flow. Confirmed via `grep -rn color e2e/*.spec.ts`: only that one unrelated fixture and unrelated `accessibility.spec.ts` comments about color-contrast scanning. **Not run**: the full `npm run e2e` Playwright suite — it requires standing up Firebase Auth/Firestore emulators plus the Express backend concurrently, which was judged disproportionate to a single-value default-color change already covered by 2491 passing unit/component tests, type-check, and lint; flagged here rather than claimed.
- [ ] T010 Execute `quickstart.md` end-to-end in the browser (`npm run dev`): §2 golden path across all columns/templates, §3 manual-override regression, and §4 existing-data-unaffected regression. **Not performed as a live authenticated browser session** — the app requires Google-OAuth-emulator sign-in against a running Firestore/Auth emulator + backend, out of proportion for this change's risk. Confidence instead comes from `GroupableColumn.test.tsx`'s new/updated component tests, which exercise the real `handleCreateCard`/`handleCancelCreate` code paths via React Testing Library `user-event` clicks exactly as quickstart.md §2/§3 describe, just without real Firestore persistence or pixel rendering.
