...
export interface ResetCountdownDeps {
    boardReadPort: BoardReadPort;
    countdownPort: CountdownPort;
}

export interface ResetCountdownParams {
    boardId: string;
    requesterUid: string;
}

/** contracts/facilitator-tools-api.md `POST /api/boards/:id/countdown/reset` — facilitator only (FR-004). */
export async function resetCountdown(deps: ResetCountdownDeps, params: ResetCountdownParams): Promise<CountdownTimer> {
    const board = await deps.boardReadPort.getBoard(params.boardId);
    if (!board) throw new NotFoundError('Board not found');
    if (!isFacilitator(board, params.requesterUid)) {
        throw new ForbiddenError('Only the board facilitator may reset the countdown timer');
    }
    const timer = await deps.countdownPort.getTimer(params.boardId);
    if (!timer) throw new NotFoundError('Timer not found');
    return deps.countdownPort.resetTimer(params.boardId);
}
